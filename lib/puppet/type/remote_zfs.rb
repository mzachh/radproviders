...
parent zfs instances, the zfs resource will autorequire them."
    ...
    newparam(:rad_address) do
      desc "Address of the RAD REST web service of the ZFS server. For example `zfsserver1.example.com:12304` "
    end

    newparam(:verify_ssl) do
      desc "Deactivate SSL verification of the HTTPS connection to RAD server. Valid values are `true`, `false`."
      defaultto :true
